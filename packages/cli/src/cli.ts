#!/usr/bin/env node
import {readFileSync,readdirSync,existsSync} from 'node:fs'; import path from 'node:path'; import {loadProfile,validateProfile,validateNormalizedResult,loadResult,retentionPlan,sha256,correlateTrigger,assuranceSummary} from '@rahp/core'; import {RahpGraph} from '@rahp/graph';
...
if(cmd==='correlate-trigger'){const v=JSON.parse(readFileSync(a[0],'utf8'));console.log(JSON.stringify(correlateTrigger(v.observation,v.open_assessments||[]),null,2))}
if(cmd==='assurance-summary'){const r=validateNormalizedResult(a[0]);if(!r.valid)die(r.errors.join('\n'));console.log(JSON.stringify(assuranceSummary(loadResult(a[0])),null,2))}
...
if(!cmd||cmd==='describe'){console.log('RAHP TypeScript reference SDK candidate v1.2 assurance extensions\nCommands: validate-profile targets validate-result retention-plan assurance-summary correlate-trigger sha256 trace graph-stats conformance')}
