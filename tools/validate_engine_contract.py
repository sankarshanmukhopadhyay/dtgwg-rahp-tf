...
except ImportError:sys.exit('requires PyYAML and jsonschema')
ROOT=pathlib.Path(__file__).resolve().parent.parent;sys.path.insert(0,str(ROOT/'tools'))
from engine_contract import validate_result,load_result,retention_plan,correlate_trigger
...
def main():
    errors=0;contract=yaml.safe_load((ROOT/'method/engine-contract.yaml').read_text());stages=contract.get('execution_model',{}).get('stages') or []
    ...
    ops={x.get('id') for x in contract.get('operations') or []};required={'validate-profile','observe-source','correlate-trigger','scaffold-assessment','classify-evidence','evaluate-assurance','validate-assessment','normalize-result','plan-retention','plan-remediation','evaluate-retest','promote-disposition'}
    if not required<=ops:errors+=fail('engine contract missing operations: '+', '.join(sorted(required-ops)))
    policy=yaml.safe_load((ROOT/'method/evidence-retention.yaml').read_text());classes=policy.get('classes') or {}
    ...
    for result_path in fixtures:
        exp=yaml.safe_load((result_path.parent/'expected.yaml').read_text());actual=validate_result(result_path,quiet=True)
