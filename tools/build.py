...
NAV = [
    ...
    ("normative.html", "DTG standards pipeline", "var(--plum)"),
    ("matrix.html", "DTG cross-reference matrix", "var(--teal)"),
    ("lifecycle.html", "DTG lifecycle & gaps", "var(--amber)"),
    ("governance.html", "DTG decisions & acceptances", "var(--green)"),
    ("assurance.html", "DTG operational assurance", "var(--teal)"),
    ("task-force-actions.html", "DTG Task Force actions", "var(--amber)"),
]
...
<title>{html.escape(title)} — RAHP Toolkit</title>
<link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@300;400;500;600&family=DM+Mono:wght@400;500&display=swap" rel="stylesheet">
<style>{CSS}</style></head><body><div class="layout">
<aside class="sidebar">
  <div class="sb-brand"><span class="sb-wordmark">RAHP Toolkit</span>
  <span class="sb-project">Portable assurance + DTG instance</span></div>
...
<p class="gen">generated by tools/build.py · portable method/catalogue + DTG deployment data · {meta['counts']}</p></div>
...
<div class="masthead"><div class="w">RAHP Toolkit</div><h2>Portable assurance + DTG deployment views
...
document.addEventListener('click', function(e){{
  ...
  const parentPane=target.closest('.pane');
  if(parentPane && !parentPane.classList.contains('on')){{
    document.querySelectorAll('.pane').forEach(p=>p.classList.remove('on'));
    document.querySelectorAll('.tabbtn').forEach(b=>b.classList.remove('on'));
    parentPane.classList.add('on');
    const paneKey=parentPane.id.slice(5);
    const buttons=[...document.querySelectorAll('.tabbtn')];
    const paneButton=buttons.find(b=>b.getAttribute('onclick')?.includes("'"+paneKey+"'"));
    if(paneButton) paneButton.classList.add('on');
...
def portable_pill(identifier):
    prefix = identifier.split('-', 1)[0]
    cls = {'HRM':'prk','RKP':'prk','CTP':'pct','GRP':'pgr','ATP':'pat','EVP':'psc'}.get(prefix, 'pm')
    safe = html.escape(identifier)
    return f'<a href="portable-catalogue.html#{safe}" class="ref-link"><span class="pill {cls}">{safe}</span></a>'


def portable_pills(ids):
    return ''.join(portable_pill(i) for i in (ids or [])) or '<span style="color:var(--faint)">—</span>'


def portable_catalogue_body(catalogue):
    records = catalogue['records']
    specs = [
        ('harm_patterns','Human harms','HRM','definition'),
        ('risk_patterns','Risk patterns','RKP','description'),
        ('control_patterns','Control patterns','CTP','control_objective'),
        ('guardrail_patterns','Guardrails','GRP','prohibited_state'),
        ('assurance_patterns','Assurance patterns','ATP','assertion'),
        ('evidence_patterns','Evidence contracts','EVP','claim_supported'),
    ]
    out = ['<div class="card"><div class="cb"><strong>Portable method layer.</strong> These reusable records live under <code>method/catalogue/</code>. They complement rather than replace deployment-specific <code>RK-*</code>, <code>CT-*</code>, <code>GR-*</code> and governance records.</div></div>']
    relationship_fields = ('harm_patterns','risk_patterns','control_patterns','guardrail_patterns','evidence_patterns')
    for key, title, prefix, summary_field in specs:
        out.append(f'<div class="sh2"><h2>{title}</h2><span class="cnt">{len(records[key])} {prefix} records</span></div>')
        for r in records[key]:
            rel=[]
            for f in relationship_fields:
                vals=r.get(f) or []
                if vals:
                    rel.append(f'<div style="margin-top:8px"><small class="mono">{html.escape(f.replace("_"," "))}</small><br>{portable_pills(vals)}</div>')
            meta=[]
            for f in ('family','control_function','protected_interest','assurance_level','freshness','privacy_classification','authority','enforcement_point'):
                if r.get(f): meta.append(f'<span class="pill pm">{html.escape(str(r[f]))}</span>')
            out.append(f'<div class="card ref-card" id="{html.escape(r["id"])}"><div class="cb"><div class="ref-title">{portable_pill(r["id"])}<strong>{html.escape(r.get("name") or "")}</strong></div><div class="ref-summary">{html.escape(str(r.get(summary_field) or ""))}</div><div class="ref-meta">{"".join(meta)}</div>{"".join(rel)}</div></div>')
    return ''.join(out)
...
def assurance_graph_body(catalogue, head_qualification):
    ...
    evidence_ids = {e['id'] for e in rec['evidence_patterns']}
    used_evidence = {e for t in rec['assurance_patterns'] for e in (t.get('evidence_patterns') or [])}
    body = '<div class="card"><div class="cb"><strong>Portable assurance chain.</strong> <code>HRM → RKP → CTP/GRP → ATP → EVP</code>. Deployment findings and <code>RK-*</code> risks map into this graph without automatically becoming portable method records.</div></div>'
    body += '<div class="pgrid">' + ''.join([
        f'<div class="pcard"><h3>{len(rec["harm_patterns"])}</h3><div class="role">portable harms</div></div>',
        f'<div class="pcard"><h3>{len(rec["risk_patterns"])}</h3><div class="role">portable risks</div></div>',
        f'<div class="pcard"><h3>{len(rec["control_patterns"])}</h3><div class="role">portable controls</div></div>',
        f'<div class="pcard"><h3>{len(rec["guardrail_patterns"])}</h3><div class="role">portable guardrails</div></div>',
        f'<div class="pcard"><h3>{len(rec["assurance_patterns"])}</h3><div class="role">assurance patterns</div></div>',
        f'<div class="pcard"><h3>{len(rec["evidence_patterns"])}</h3><div class="role">evidence contracts</div></div>',
    ]) + '</div>'
    checks = [
    ...
    rows=''.join(f'<tr><td>{html.escape(label)}</td><td>{len(vals)}</td><td>{portable_pills(vals)}</td></tr>' for label, vals in checks)
    body += f'<div class="sh2"><h2>Coverage diagnostics</h2></div><div class="card"><table><thead><tr><th>Check</th><th>Count</th><th>Records</th></tr></thead><tbody>{rows}</tbody></table></div>'
    q = (head_qualification or {}).get('qualification') or {}
    if q:
        sm=q.get('summary') or {}
        classes=defaultdict(int)
        for findings in (q.get('finding_deltas') or {}).values():
            for finding in findings: classes[finding.get('classification','unknown')]+=1
        body += f'<div class="sh2"><h2>HEAD qualification evidence</h2></div><div class="card"><div class="cb"><strong>{html.escape(q.get("id") or "")}</strong> · {html.escape(q.get("run_date") or "")}<div class="ref-summary">{html.escape(sm.get("overall") or "")}</div><div class="ref-meta"><span class="pill pct">{sm.get("live_repositories",0)} live repos</span><span class="pill psc">{sm.get("head_unchanged_from_canonical_baseline",0)} at baseline HEAD</span><span class="pill pat">{sm.get("head_advanced",0)} advanced</span><span class="pill sev-Low">{classes.get("resolved",0)} resolved</span><span class="pill sev-Medium">{classes.get("weakened",0)} weakened</span><span class="pill prk">{sm.get("new_open_findings",0)} new open</span></div></div></div>'
    return body
...
def build_site(records, derived, meta, out):
    ...
    body = portable_catalogue_body(derived["portable_catalogue"])
    ...
    (site / "assurance-graph.html").write_text(page("assurance-graph.html", ptitle, psub, body, meta), encoding="utf-8")
    pages.append(("assurance-graph", ptitle, psub, body))
...
{f'<div class="lab">Evidence</div><ul style="padding-left:16px">{ev}</ul>' if ev else ''}
    ...
    guide = """<div class="card"><div class="cb"><strong>Two layers, one assurance system.</strong> The portable method catalogue defines reusable <code>HRM/RKP/CTP/GRP/ATP/EVP</code> patterns. The DTG deployment catalogue under <code>data/</code> is one concrete instance with local <code>RK/CT/GR/AT</code> records, governance state and operational evidence.</div></div>
<div class="sh2"><h2>Start here</h2></div>
...
<td>{mon}</td></tr>""")
    ...
    ptitle, psub = "DTG risks & metrics", "Deployment-specific DTG risk and metric records. Portable RKP/HRM patterns are shown separately in the portable assurance catalogue."
    ...
    body = ('<div class="card"><div class="cb"><strong>DTG deployment catalogue.</strong> '
            'Every deployment identifier in generated DTG-facing output links here. Portable method patterns use the separate <a href="portable-catalogue.html">portable assurance catalogue</a>. Each deployment record has a stable '
            'fragment identifier, so links such as <code>catalogue.html#RK-AI01</code> remain durable across builds.</div></div>'
            + "".join(sections))
    ptitle, psub = "DTG deployment catalogue", "Stable, deep-linkable DTG instance records from data/. This is deployment evidence, not the complete portable RAHP method catalogue."
...
<td><span class="pill sev-Medium">{html.escape(r.get('status') or '')}</span></td></tr>""" for r in records["recommendation"])
    ...
    ptitle, psub = "DTG standards pipeline", "Deployment-specific standards actions and triage state derived from the DTG instance catalogue."
    ...
    ptitle, psub = "DTG cross-reference matrix", "The DTG deployment identifier space linking personas, stories, scenarios, EPICs and local risks."
...
<dt>Evidence required</dt><dd>{html.escape(s.get('evidence_required') or '—')}</dd>
    ...
    ptitle, psub = "DTG lifecycle & gaps", "The portable RAHP lifecycle applied to the DTG deployment, with DTG-specific gaps shown as tracked records."
...
<td><span class="pill sev-Medium">{html.escape(g['status'])}</span></td></tr>""" for g in records["governance_precedent"])
    ...
    ptitle, psub = "DTG decisions & acceptances", "Deployment-owned risk acceptances and governance precedents. These records do not establish toolkit-wide governance authority."
    ...
    body = (
        ...
        'These DTG monitoring and governance records demonstrate deployment-level operational assurance. The v1.1 portable ATP/EVP model is shown on the Assurance graph page. '
    ...
    ptitle, psub = "DTG operational assurance", "DTG deployment monitoring contracts, evidence artefacts and governance rule profiles, shown alongside the separate v1.1 portable ATP/EVP assurance model."
    ...
    ptitle, psub = "DTG Task Force action register", "A derived queue of DTG deployment records awaiting accountable Task Force decisions; it is not a toolkit-global work queue."
